...
namespace HairBand
{
    public class HairBandFileSystem : DotLiquid.FileSystems.IFileSystem
    {

        public string Root { get; set; }

        public HairBandFileSystem(string root)
        {
            Root = root;
        }
        ...
        public string FullPath(string templatePath)
        {
            ...
            {
                fullPath = templatePath.Contains("/")
                    ? Path.Combine(Path.Combine(Root, Path.GetDirectoryName(templatePath)), string.Format("{0}.riff", Path.GetFileName(templatePath)))
                    : Path.Combine(Root, string.Format("{0}.riff", templatePath));
